...
// função que acha onde a rebeka esta
...
// roda a lógica da onda e imprimir o resultado final
void executarFuga(int linha, int coluna, char mapa[linha][coluna + 1], int distancias[linha][coluna]) {
  ...
  // O laço roda enquanto a puder andar E a saída não for encontrada
  while (atualizacao == 1 && achouSaida == 0) {
    ...
    // so aumentamos o passo para o próximo turno se ainda não achamos a saída
    if (achouSaida == 0) {
      passo++;
    }
  }
